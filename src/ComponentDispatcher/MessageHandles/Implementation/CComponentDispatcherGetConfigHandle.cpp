...
TIMM_OSAL_ERRORTYPE CComponentDispatcherGetConfigHandle::Process(void* pMessage)
{
    int eError = 0;
    ...
    for (;nOffset < nPayloadSize;)
	{
		...
		eError = pComp->GetConfig((OMX_INDEXTYPE)pConfig->nStructureId, pConfig->pConfigData);
    ...
    MMS_IL_PRINT("OMX_GetConfig - 0x%08x\n", eError);
    return eError;
}